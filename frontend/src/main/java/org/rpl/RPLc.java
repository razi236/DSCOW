...
import org.rpl.frontend.typechecker.CheckSPLCommand;
import org.rpl.frontend.typechecker.locationtypes.LocationType;

import org.rpl.frontend.typechecker.nullable.NullableType;
...
@Command(name = "absc",
         ...
         versionProvider = RPLc.AbscVersionProvider.class
         )
public class RPLc implements Callable<Integer> {
    ...
    public static class Backend {
        ...
        @Option(names = { "-l" }, required = true,
            description = "@|bold Rpl to list of classes and methods:|@ translate Rpl into list of classes and methods and exit")
        public boolean methodlist = false;
    ...
    static class AbscVersionProvider implements IVersionProvider {
        public String[] getVersion() throws Exception {
            String version = RPLc.class.getPackage().getImplementationVersion();
            String gitversion = RPLc.class.getPackage().getSpecificationVersion();
            if (version == null)
                version = "HEAD";
            if (gitversion == null)
     ...
     * dispatched via {@link org.rpl.frontend.parser.Main Main}.
     * @param args the command line options
     * @return an initialized Absc instance
     */
    public static RPLc parseArgs(String[] args) {
        return CommandLine.populateCommand(new RPLc(), args);
    }

    /**
     * The main entry point for `absc`.  Command line parsing happens
     * in this class, type-checking and dispatching to backends are
     * done by {@link org.rpl.frontend.parser.Main Main}.
     * @param args
     */
    public static void main(String[] args) {
        int exitCode = new CommandLine(new RPLc()).execute(args);
